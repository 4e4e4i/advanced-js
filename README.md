...
### Какие типы бывают в Javascript? <a name="jsTypes"></a>
...
### Какая разница между классическим и прототипным наследование в JavaScript? <a name="inheritanceClassical"></a>
...
Между тем, если foo.com возвращает другой домен. Например google.com и получается, что он отличается от значеия
...
значением метода, который вы выполняете. И браузер как бы спрашивает у foo.com "смотри я хочу отправить тебе PUT.
 ...
 Вы возможно уже слышали о JSON, и возможно использовали его, но понимаете ли вы по настоящему, что это такое? Я предпологаю
...
Так же если мы зайдем в меню разработчика в бразуера во вкладку Network и выберем отфильтровать по XHR, где показаны все
...
Эти две стрелки, что я вам показываю, являются разными фазами. Та которая идет от корня к target называется событием
...
события на каждый элемент, который на клик будет записывать в консоль элемент и его параметр event, последним параметром в
функции добавления слушателя события являтся boolean, который определяет хотите ли вы слушать capturing фазу или
bubbling phase. И если вы заметили я использовал замыкание, как вы думаете зачем? Я использовал замыкание, чтобы
вывести элемент в консоль.

Так что же будет если кликнуть на button? Когда мы кликнем на button , то в консоле увидим, что сначала появится элемент
button, затем third item - его родитель и так далее до body, если мы поменяем фазу на capturing, то изначально у нас
будет body, затем first и так далее до button.

### Какая разница между stopPropagation() и preventDefault()? <a name="stopProp&prevDef"></a>

stopPropagation на самом деле останавливает событие от продолжения вниз, если это capturing фаза или вверх, если это bubbling
фаза. 

Посмотрим на примере, который очень похож на предыдущий, только с мелкими изменениями:
...
<html lang="en">
   ...
   <body class="body item">
     <div class="first item">
       <div class="second item">
           <div class="third item">
               <input type="checkbox" class="fourth item">Hello<br>
           </div>
       </div>
     </div>
...
Причина по которой я добавил alert, чтобы объяснить вам событие не распростроняется, он не перемещается на следующий
слушатель пока предыдущий не закончит. Например после клика по чекбксу, появится алерт и пока мы не нажмем OK, следущий не 
будет запускаться.

Теперь запустим код с stopPropagation например с уровня 2
...
Теперь после второго элемента наши слушатели не запускаются. Тоже самое с bubbling.

А что же тогда делает preventDefault? 
...
Он не останавливает событие от смещения по пути от корневого элемента до таргета. Но если мы посмотрим внимательно, то
увидим, что наш checkbox не перешл в состояние checked. Так preventDefault останавливает поведение по-умолчанию, то событие
которое должно было сработать на элементе во время действия